...
- (void)refreshWithMember:(NTESContactDataMember *)member{
    self.member = member;
    self.textLabel.text = [NTESSessionUtil showNick:member.kitInfo.infoId inSession:nil];
    [self.textLabel sizeToFit];
    NSURL *url;
    if (member.kitInfo.avatarUrlString.length) {
        url = [NSURL URLWithString:member.kitInfo.avatarUrlString];
    }
    [_avatarImageView nim_setImageWithURL:url placeholderImage:member.kitInfo.avatarImage];
}


- (void)onTouchAvatar:(id)sender{
    if ([self.delegate respondsToSelector:@selector(didTouchUserListAvatar:)]) {
        [self.delegate didTouchUserListAvatar:self.member.kitInfo.infoId];
    }
}
